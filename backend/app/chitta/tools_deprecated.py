...
For extraction functions, use an `info_categories` or similar field.

Example:
    BAD:  "required": []  with 20+ optional properties → 400 error
    GOOD: "required": ["info_categories"]  → works fine

If you see Gemini 400 errors about "schema produces a constraint that has
too much branching", check that the function schema has at least one
required field.
...
""",
        "parameters": {
            "type": "object",
            "properties": {
                # === REQUIRED: Categories being extracted (breaks branching) ===
                "info_categories": {
                    "type": "array",
                    "items": {
                        "type": "string",
                        "enum": ["identity", "essence", "strengths", "history", "family", "concerns", "other"]
                    },
                    "description": "Which categories of information are being extracted in this call"
                },
                # === Identity ===
                "child_name": {
                    "type": "string",
